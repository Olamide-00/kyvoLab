...
function AlgorithmCanvas({ dark = false }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const orbs = [
        {
          x: LW * 0.75 + Math.sin(t.current * 0.4) * 60,
          y: LH * 0.2 + Math.cos(t.current * 0.3) * 40,
          r: 280,
          c: TEAL,
          op: dark ? 0.09 : 0.07,
        },
        {
          x: LW * 0.15 + Math.cos(t.current * 0.35) * 50,
          y: LH * 0.65 + Math.sin(t.current * 0.45) * 50,
          r: 220,
          c: BLUE,
          op: dark ? 0.07 : 0.055,
        },
        {
          x: LW * 0.5 + Math.sin(t.current * 0.25) * 80,
          y: LH * 0.85,
          r: 180,
          c: TEAL,
          op: dark ? 0.05 : 0.04,
      ...
      ctx.lineWidth = 0.6;
      const gSpacing = 64;
      for (let x = 0; x < LW; x += gSpacing) {
        ctx.beginPath();
        ctx.moveTo(x, 0);
        ctx.lineTo(x, LH);
        ctx.stroke();
      }
      for (let y = 0; y < LH; y += gSpacing) {
        ctx.beginPath();
        ctx.moveTo(0, y);
        ctx.lineTo(LW, y);
        ctx.stroke();
        ...
        const tip =
          full < total
            ? {
                x:
                  circ.segs[full].x +
                  (circ.segs[full + 1].x - circ.segs[full].x) * frac,
                y:
                  circ.segs[full].y +
                  (circ.segs[full + 1].y - circ.segs[full].y) * frac,
              }
            : circ.segs[total];
      ...
      glyphs.forEach((g) => {
        ...
        if (g.y < -20) {
          g.y = LH + 20;
          g.text = CODE_LINES[Math.floor(Math.random() * CODE_LINES.length)];
        }
    ...
    const ro = new ResizeObserver(() => {
      LW = c.offsetWidth;
      LH = c.offsetHeight;
      c.width = LW * devicePixelRatio;
      c.height = LH * devicePixelRatio;
      ctx.scale(devicePixelRatio, devicePixelRatio);
    });
    ro.observe(c);
    draw();
      ...
      >
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 7, color: p.ac, fontWeight: 700 }}>
              ● LIVE
            </div>
          </div>
          {/* Balance card */}
          <div
            style={{
              ...
              background: "linear-gradient(135deg,#0E2540,#071830)",
          ...
          >
            <div
              style={{
                ...
                right: 0,
                height: 2,
                background: `linear-gradient(90deg,${p.ac},#1A8FFF)`,
              }}
            />
            <div
              style={{
          ...
          >
            {[
              ...
              >
                <div style={{ fontSize: 11, color: a.c, fontWeight: 700 }}>
                  {a.l}
                </div>
                <div
                  style={{
            ...
            {[35, 55, 42, 65, 50, 72, 60].map((h, i) => (
              <div
                ...
                style={{
                  ...
                  background:
                    i === 6
                      ? `linear-gradient(180deg,${p.ac},${p.ac}44)`
                      : `linear-gradient(180deg,#1A8FFF22,#1A8FFF08)`,
                }}
              />
            ))}
          </div>
          {/* Transactions */}
          {[
            { l: "Airtime", v: "-₦500", c: "#FF4D6D" },
            { l: "Transfer in", v: "+₦15K", c: p.ac },
          ].map((tx) => (
            <div
              key={tx.l}
              style={{
            ...
            >
              ...
              <span style={{ fontSize: 7.5, fontWeight: 700, color: tx.c }}>
                {tx.v}
              </span>
            </div>
          ))}
...
const PROJECTS = [
  {
    ...
    features: [
      ...
      "Overdraft & credit scoring",
      "CBN compliance module",
      "In-app customer support",
      "Fraud detection engine",
    ],
    metrics: [
      { n: "50K+", l: "Daily active users" },
      { n: "₦2.8B", l: "Monthly volume" },
      { n: "99.9%", l: "Uptime SLA" },
    ],
  },
  {
    ...
    features: [
      ...
      "Automated CBN reporting",
      "Multi-branch management",
      "Teller & vault operations",
      "GL & reconciliation engine",
    ],
    metrics: [
      { n: "12K+", l: "Active accounts" },
      { n: "₦800M", l: "Loans disbursed" },
      { n: "4", l: "Branches live" },
    ],
  },
  {
    ...
    features: [
      ...
      "Merchant reseller portal",
      "Margin & pricing engine",
      "Automated reconciliation",
      "SMS & push notifications",
    ],
    metrics: [
      { n: "200K+", l: "Transactions/month" },
      { n: "9", l: "Billers integrated" },
      { n: "1.2s", l: "Avg response time" },
    ],
  },
  {
    id: 4,
    ...
    features: [
      "Instant NIP transfers",
      "Bill splitting with groups",
      "Money requests via link",
      "Scheduled payments",
  ...
  },
  {
    ...
    features: [
      ...
      "SEC compliance module",
    ],
    metrics: [
      { n: "18K+", l: "Investors" },
      { n: "$2.4M", l: "Assets under mgmt" },
      { n: "12.4%", l: "Avg annual yield" },
    ],
  },
  {
    id: 6,
  ...
  },
  {
    ...
    metrics: [
      { n: "22K+", l: "Verified traders" },
      { n: "$8.4M", l: "OTC volume/month" },
      { n: "SEC", l: "Registered" },
    ],
  },
  {
    ...
    features: [
      ...
      "BNPL checkout widget",
      "Debt collection module",
      "CRC bureau integration",
      "Loan portfolio dashboard",
    ],
    metrics: [
      { n: "₦2.1B", l: "Loans disbursed" },
      { n: "94%", l: "Repayment rate" },
      { n: "8 sec", l: "Decision time" },
    ],
  },
];
...
const SERVICES = [
  {
    ...
    desc: "End-to-end digital bank builds — account opening, KYC, core banking, lending, and CBN compliance.",
    tags: ["CBS", "KYC/AML", "CBN"],
  },
  {
    icon: "🏛️",
    title: "MFB Core Banking",
    desc: "Complete CBS for licensed MFBs — loans, savings, cooperative management, and automated CBN reporting.",
    tags: ["Core Banking", "Loans", "Reporting"],
  },
  {
    icon: "⚡",
    title: "VTU & Utility Platforms",
    desc: "Scalable VTU aggregators for airtime, data, bills and betting — white-label ready for any market.",
    tags: ["VTU", "Bill Payment", "Agents"],
  },
  {
    icon: "💳",
    title: "Fintech & Payment Apps",
    desc: "P2P payments, wallets, investment platforms, and embedded finance with modern mobile and web stacks.",
    tags: ["Mobile", "Payments", "Wallets"],
  },
  {
    icon: "📡",
    title: "Financial APIs & Infrastructure",
    desc: "Core financial APIs — ledger, reconciliation, FX, and transaction processing to power your fintech.",
    tags: ["REST APIs", "Webhooks", "SDKs"],
  },
  {
    icon: "🔐",
    title: "Compliance & Security",
    desc: "AML/KYC, fraud detection, biometric auth, and CBN/SEC regulatory reporting in every product we ship.",
    tags: ["AML", "Fraud", "BVN/NIN"],
  },
];
    ...
    >
      <div className="nav-mark">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
          ...
          <polygon
            points="8,12 8,4.5 18.5,4 14,12"
            fill="url(#lg)"
            opacity=".95"
          />
          <polygon
            points="8,12 8,19.5 18.5,20 14,12"
            fill="url(#lg)"
            opacity=".82"
          />
        </svg>
  ...
  return (
    <>
      ...
      <section id="home" className="hero">
        ...
        <div
          style={{
            ...
            inset: 0,
            background:
              "linear-gradient(135deg,rgba(242,244,248,.85) 0%,rgba(242,244,248,.7) 50%,rgba(242,244,248,.4) 100%)",
            zIndex: 1,
          }}
        />
        ...
        <div className="hero-inner" style={{ position: "relative", zIndex: 2 }}>
          ...
          <div>
            ...
            >
              Neobanks. MFB platforms. VTU apps. Payment infrastructure. Built
              from scratch by engineers who understand African fintech deeply.
            </p>
            <div
              className="hero-btns"
            ...
            >
              ...
              <button className="btn-s" onClick={() => go("contact")}>
                Talk to us
              </button>
            </div>
            <div
            ...
            >
              {[
                { n: "8+", l: "Products shipped" },
                { n: "₦12B+", l: "Volume processed" },
                { n: "3", l: "Countries" },
              ...
              <div className="terminal">
                ...
                <div className="term-body" style={{ fontSize: 12 }}>
                  ...
                  <div>
                    ...
                    <span className="tc-yellow">NeoBankService</span>{" "}
                    <span className="tc-white">{"{"}</span>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <span className="tc-white">walletSvc: </span>
                  ...
                  <div>
                    ...
                    <span className="tc-white">this.walletSvc.</span>
                    <span className="tc-teal">findWithLock</span>
                    <span className="tc-white">(from, mgr);</span>
                  </div>
                  <div>
                    ...
                    <span className="tc-yellow">BadRequestException</span>
                    <span className="tc-white">();</span>
                  </div>
                  <div>
                    ...
                    <span className="tc-white">(from, ₦amt, mgr);</span>
                  </div>
                  <div>
                    ...
                    <span className="tc-white">(to, ₦amt, mgr);</span>
                  </div>
                  <div>
                    <span className="tc-dim">18 </span>{" "}
                    <span className="tc-blue">return</span>{" "}
                    <span className="tc-white">{"{ status: "}</span>
                    <span className="tc-green">'success'</span>
                    <span className="tc-white">{", ref: uuid() };"}</span>
                  </div>
                  <div>
                  ...
                  <div>
                    <span className="tc-dim">20 </span>{" "}
                    <span className="tc-white">{"}"}</span>
                  </div>
                  <div>
                    <span className="tc-dim">21 </span>
                    <span className="tc-white">{"}"}</span>
                  </div>
                  <div style={{ marginTop: 8 }}>
                    ...
                    <span className="tc-teal">
                      <Typed
                        strings={[
                          "Transfer committed — ₦50,000 in 1.2s",
                          "✓ Wallet debited & credited atomically",
                          "✓ 0 failed txns this session",
                        ]}
                        speed={45}
                      />
                    </span>
      ...
      <section id="services" className="sec svc-bg">
        ...
        <div
          style={{
            position: "absolute",
            inset: 0,
            background: "rgba(236,238,244,.94)",
            zIndex: 1,
          }}
        />
        <div className="sec-in">
          ...
          <Reveal delay={80}>
            <h2 className="sec-h2">
              Engineering-first
              <br />
              <GT c="fintech products" />
            </h2>
          </Reveal>
          <Reveal delay={140}>
            <p className="sec-sub">
              We don't just write code. We architect financial systems that
              scale, comply, and deliver real business value from day one.
            </p>
          </Reveal>
        ...
        <div className="sec-in">
          ...
          <Reveal delay={140}>
            <p className="sec-sub" style={{ marginBottom: 44 }}>
              Every project is a fully engineered, production-deployed financial
              product. Click any card to see full details.
            </p>
          </Reveal>
          <Reveal delay={160}>
            <div className="proj-filters">
              {[
                ["all", "All projects"],
                ["neobank", "Neobank"],
                ["mfb", "MFB / Core Banking"],
                ["vtu", "VTU"],
                ["fintech", "Fintech Apps"],
              ].map(([v, l]) => (
                <button
                  key={v}
                  className={`pf ${filter === v ? "on" : ""}`}
                  onClick={() => setFilter(v)}
                >
                  {l}
                </button>
      ...
      {proj && (
        <div
          className="overlay"
          onClick={(e) => e.target === e.currentTarget && setProj(null)}
        >
          <div className="modal">
            <button className="modal-x" onClick={() => setProj(null)}>
              ✕
            </button>
      ...
      <section id="referral" className="sec ref-bg">
        ...
        <div className="sec-in" style={{ position: "relative", zIndex: 2 }}>
          <div className="ref-grid">
            <div>
              <Reveal>
                <div className="eyebrow" style={{ color: "#00C9A7" }}>
              ...
              <Reveal delay={80}>
                <h2 className="ref-h2">
                  Refer a client.
                  <br />
                  <GT c="Earn as they pay." />
                </h2>
              </Reveal>
              <Reveal delay={140}>
                <p className="ref-sub">
                  Know a startup or company that needs a fintech product?
                  Introduce them to us — and earn 5% of every payment they make
                  to KyvoLab. Per payment. No cap.
                </p>
              </Reveal>
              <div>
                {[
                  {
                    ...
                    d: "Connect us with any company needing a neobank, MFB platform, VTU app, or any fintech product built.",
                  },
                  {
                    n: "2",
                    t: "We close and build it",
                    d: "KyvoLab handles proposal, contracts, and full engineering end-to-end.",
                  },
                  {
                    n: "3",
                  ...
                  },
                  {
                    ...
                    d: "Calculated on every payment, paid to you within 48hrs. No ceiling. Forever.",
                  },
                ].map((s, i) => (
            ...
            <Reveal delay={200}>
              <MagCard>
                <div className="rcard">
                  ...
                  <div className="rpct-l">
                    // per_payment · no_cap · lifetime
                  </div>
                  <div className="rexample">
                    {[
                      ...
                      <div key={r.l} className="rex-row">
                        ...
                        <div className={`rex-v ${r.earn ? "earn" : ""}`}>
                          {r.v}
                        </div>
                      </div>
                    ))}
                  ...
                  <input
                    ...
                    value={rForm.name}
                    onChange={(e) =>
                      setRForm({ ...rForm, name: e.target.value })
                    }
                  />
                  <input
                    className="rinput"
                  ...
                  <input
                    ...
                    value={rForm.cname}
                    onChange={(e) =>
                      setRForm({ ...rForm, cname: e.target.value })
                    }
                  />
                  <input
                    className="rinput"
                    ...
                    value={rForm.cemail}
                    onChange={(e) =>
                      setRForm({ ...rForm, cemail: e.target.value })
                    }
                  />
                  <button
                    className="rsubmit"
                    onClick={() => {
                      if (rForm.name && rForm.email && rForm.cname) {
      ...
      <section id="commission" className="sec comm-bg">
        ...
        <div className="sec-in">
          ...
          <Reveal delay={140}>
            <p className="sec-sub">
              No hidden terms. No thresholds. Every time your referred client
              pays KyvoLab, 5% routes directly to you — for the life of the
              contract.
            </p>
          </Reveal>
          <div className="comm-grid">
            {[
              ...
              },
              {
                icon: "♾️",
                t: "Zero ceiling",
                d: "No cap on earnings. Refer multiple clients, earn on all simultaneously. More clients = more passive income.",
              },
              {
                icon: "⚡",
                t: "48-hour bank transfer",
                d: "Commission calculated when payment clears. Funds hit your bank account within 48 hours, every time.",
              },
              {
                icon: "🔁",
                t: "Lifetime earnings",
              ...
              },
              {
                ...
                d: "Track every client, every payment, and every commission earned in real time via your referral dashboard.",
              },
              {
                icon: "🏆",
                t: "Performance bonuses",
                d: "3+ clients/quarter → 7%. 5+ clients → 10%. Top referrers get a rate upgrade, not just a thank you.",
              },
            ].map((c, i) => (
            ...
            <div>
              ...
              <Reveal delay={140}>
                <p className="sec-sub" style={{ marginBottom: 40 }}>
                  Full spec or just an idea — we'll scope it, price it, and
                  build it right. First call is free.
                </p>
              </Reveal>
              {[
                {
              ...
              <div className="cform">
                ...
                <div className="frow">
                  <div className="ff">
                    ...
                    <input
                      ...
                      value={cForm.fn}
                      onChange={(e) =>
                        setCForm({ ...cForm, fn: e.target.value })
                      }
                    />
                  </div>
                  <div className="ff">
                    ...
                    <input
                      ...
                      value={cForm.ln}
                      onChange={(e) =>
                        setCForm({ ...cForm, ln: e.target.value })
                      }
                    />
                  </div>
                </div>
                ...
                <div className="ff">
                  ...
                  <select
                    ...
                    value={cForm.svc}
                    onChange={(e) =>
                      setCForm({ ...cForm, svc: e.target.value })
                    }
                  >
                ...
                <div className="ff">
                  ...
                  <textarea
                    ...
                    value={cForm.msg}
                    onChange={(e) =>
                      setCForm({ ...cForm, msg: e.target.value })
                    }
                  />
                </div>
                <button
                  className="fsub"
                  onClick={() => {
                    if (cForm.fn && cForm.email && cForm.msg) {
        ...
        <div className="fi-in">
          <div className="fg">
            <div>
              <Logo />
              <p className="fb-desc">
                KyvoLab engineers financial software for startups and
                enterprises across Africa. From concept to production — we build
                fintech that actually works.
              </p>
              <div className="fsocs">
                {["📸", "👥", "💼", "𝕏"].map((s, i) => (
                  <div key={i} className="fsoc">
                    {s}
                  </div>
                ))}
              </div>
          ...
          <div className="fbot">
            ...
            <div className="fblnks">
              {["privacy_policy", "terms_of_service", "cookies"].map((l) => (
                <button key={l} className="fblnk">
                  {l}
                </button>
              ))}
            </div>
